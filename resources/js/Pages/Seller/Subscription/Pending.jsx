...
export default function Pending() {
    ...
    useEffect(() => {
        const channel = window.Echo.private(`seller-subscription-confirmed.${user?.id}`)
            .listen('.SellerSubscriptionConfirmed', (e) => {
                console.log('🔔 New subscription confirmed received!', e);
                setToast({ show: true, message: "Your subscription has been confirmed successfully!", type: "success" });
                window.location.href = route("seller.subscription.landing");
            });



        return () => {
            channel.stopListening('.SellerSubscriptionConfirmed');
        };
    }, [user?.id]);
    return (
        <SellerLayout>
